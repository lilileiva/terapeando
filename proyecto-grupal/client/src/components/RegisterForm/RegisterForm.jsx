...
function RegisterForm() {
  ...
  const validate = (signupForm) => {
    ...
    if (
      ...
      (Date.now() - new Date(signupForm.birthdate)) / 31557600000 < 18
    ) {
  ...
  const handleSpecialities = (e) => {
    setSignupForm({
      ...signupForm,
      specialities: [
        ...signupForm.specialities.filter((s) => s !== e.target.value),
        e.target.value,
      ],
    });
  };
  ...
  const afterSubmit = async () => {
    if (
      signupForm.license &&
      signupForm.dni &&
      signupForm.specialities &&
      signupForm.education &&
      Object.keys(formErrors).length === 0
    ) {
      const response = await axios.post(
        `${baseURL}/userpsychologist`,
        signupForm
      );
      ...
      const response = await axios.post(
        `${baseURL}/userclient/client/register`,
        signupForm
      );
      if (response.status === 201) {
        navigate("/signin");
                    ...
                    <>
                      ...
                      >
                        {specialitiesList.map((e) => (
                          <option key={e} name="specialities" value={e}>
                            {e}
                          </option>
                        ))}
                      </Select>
                      {formErrors.specialities && (
                        <Text fontSize="sm" color="teal.500">
                          {formErrors.specialities}
                        </Text>
                      )}
                      ...
                      <ul>
                        {signupForm.specialities
                          ? signupForm.specialities.map((e) => (
                              <Stack direction="row" margin="0.2em">
                                <Text fontSize="md" color="teal.700">
                                  {e}
                                </Text>
                                <BiX
                                  onClick={() => handleSpecialitiesDelete(e)}
                                  className="iconX"
                                />
                              </Stack>
                            ))
